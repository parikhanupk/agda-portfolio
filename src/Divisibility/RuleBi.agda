...
module Divisibility.RuleBi where



open import Data.Nat using (ℕ; zero; suc; _>_; _+_; _*_; _≥_; _∸_; _≤_; z≤n; s≤s; _^_; _<ᵇ_)
open import Data.Nat.Properties using (+-assoc; +-identityʳ; +-comm; *-comm; +-∸-assoc; *-identityʳ; *-distribˡ-∸)
...
a≥1→b≥1→a*b≥1 : ∀ (a b : ℕ) → a ≥ 1 → b ≥ 1 → (a * b) ≥ 1
a≥1→b≥1→a*b≥1 (suc a) (suc b) _ _ = s≤s z≤n



a≥b→a∸1≡[a∸b]+[b∸1] : ∀ (a b : ℕ) → a ≥ b → b > 1 → a ∸ 1 ≡ (a ∸ b) + (b ∸ 1)
a≥b→a∸1≡[a∸b]+[b∸1] (suc a) (suc b) (s≤s a≥b) (s≤s b≥1) rewrite +-comm (a ∸ b) b = a≥b→a≡b+[a∸b] a b a≥b



b>1→b*bᵃ≥b : ∀ (a b : ℕ) → b > 1 → b * b ^ a ≥ b
b>1→b*bᵃ≥b zero b _ rewrite *-identityʳ b = a≤a b
b>1→b*bᵃ≥b (suc a) b b>1 rewrite *-comm b (b * b ^ a)
                               = b≥1→a≤b*a b
                                           (b * b ^ a)
                                           (a≥1→b≥1→a*b≥1 b
                                                          (b ^ a)
                                                          (b>1→b≥1 b b>1)
                                                          (b≥1→bᵃ≥1 b a (b>1→b≥1 b b>1)))



b[bᵃ∸1]≡b[bᵃ∸b] : ∀ (a b : ℕ) → b * (b ^ a ∸ 1) ≡ b * b ^ a ∸ b
b[bᵃ∸1]≡b[bᵃ∸b] a b rewrite *-distribˡ-∸ b (b ^ a) 1
                          | *-identityʳ b
                          = refl
...
div-b∸1-bᵃ∸1 zero b b>1 = dz
div-b∸1-bᵃ∸1 (suc a) b b>1 = subst (Div (b ∸ 1) (b>1→b∸1>0 b b>1))
                                   (sym (a≥b→a∸1≡[a∸b]+[b∸1] (b * b ^ a) b (b>1→b*bᵃ≥b a b b>1) b>1))
                                   (da→db→d[a+b] {b ∸ 1} {b * b ^ a ∸ b} {b ∸ 1}
                                                 (b>1→b∸1>0 b b>1)
                                                 (subst (Div (b ∸ 1) (b>1→b∸1>0 b b>1))
                                                        (b[bᵃ∸1]≡b[bᵃ∸b] a b)
                                                        (db→d[a*b] {b ∸ 1} {b} {b ^ a ∸ 1}
                                                                   (b>1→b∸1>0 b b>1)
                                                                   (div-b∸1-bᵃ∸1 a b b>1)))
                                                 (a→da (b>1→b∸1>0 b b>1)))
