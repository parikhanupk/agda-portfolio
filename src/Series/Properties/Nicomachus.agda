...
[n+2]³+[Σ[n+1]]²≡[Σ[n+2]]² n =
  begin
    (suc (suc n)) ³ + (sum (downFrom (suc n) (s≤s z≤n))) ²
  ≡⟨ cong (_+ (sum (downFrom (suc n) (s≤s z≤n))) ²) (*-comm (2 + n) ((2 + n) ²)) ⟩
  ...
  ≡⟨ cong (_+ (sum (downFrom (suc n) (s≤s z≤n))) ²) (+-assoc ((2 + n) ²) ((2 + n) ²) ((2 + n) ² * n)) ⟩
    (2 + n) ² + ((2 + n) ² + ((2 + n) ² * n)) + (sum (downFrom (suc n) (s≤s z≤n))) ²
  ≡⟨ cong (_+ (sum (downFrom (suc n) (s≤s z≤n))) ²) (cong ((2 + n) ² +_) (sym (2*[[n+2]*Σ[n+1]]≡[n+2]²+[[n+2]²*n] n))) ⟩
...
sum-of-cubes≡square-of-sum : ∀ (n : ℕ) → (n>0 : n > 0)
                           → sum (map _³ (downFrom n n>0))
                           ≡ (sum (downFrom n n>0)) ²
